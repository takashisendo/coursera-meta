...
def validate_project_dynamic(pyproject: T) -> T:
    project_table = pyproject.get("lab3_1", {})
    dynamic = project_table.get("dynamic", [])

    for field in dynamic:
        if field in project_table:
            msg = f"You cannot provide a value for `lab3_1.{field}` and "
            msg += "list it under `lab3_1.dynamic` at the same time"
            name = f"data.lab3_1.{field}"
